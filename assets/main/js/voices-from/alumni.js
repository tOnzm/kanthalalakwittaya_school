...
app.controller('alumni',['$scope', 'daftkung', function ($scope, daftkung) {
  ...
  $scope.get_data_highlight = function get_data_highlight() {
    ...
    daftkung.get('./'+language+'/voice_from/get_alumni/highlight', [],function (res) {
  ...
  $scope.get_data = function get_data() {
    ...
    if($scope.data_stop != true && $scope.this_load != true){
      ...
      daftkung.get('./'+language+'/voice_from/get_alumni',{ 'per_page' : $scope.all_data.per_page, 'prefix': prefix  },function (res) {
        if(res.length > 0){
...
$(document).ready(function() {
  $("#header").backstretch("./assets/main/images/voices-from/alumni/header.jpg",{ 'alignY' : 'top' });
});
  ...
  if($(window).scrollTop() + $(window).height() > $(document).height() - 100) {
    setTimeout(function () {
      if($(window).scrollTop() > 500){
        if($('#alumni').scope().this_load == false){
          $('#alumni').scope().get_data();
        }
      }
    },200);
  }
